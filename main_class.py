...
def take_snapshots():
    ...
    for t in tqdm(range(1024)):
        ...
        probes.reverse()
        sb.heatmap(probes, yticklabels=False, xticklabels=False, vmin=0, vmax=1)
...
def validate_lcp():
    ...
    for _ in tqdm(range(num_samples)):
        ...
        game.run_game(enforce_paths_length=False)
        random_results.append(game.get_r_gain())
    print("\nrandom: ", (sum(random_results) / float(len(random_results))) / 1024.0)
    ...
    print("\nlcp: ", (sum(lcp_results)/float(len(lcp_results)))/1024.0)
    ...
    print("\nltr: ", (sum(ltr_results) / float(len(ltr_results))) / 1024.0)
    ...
    print("\ncircVert: ", (sum(circVert_results) / float(len(circVert_results))) / 1024.0)
    ...
    print("\nnonCircVert: ", (sum(noncircVert_results) / float(len(noncircVert_results))) / 1024.0)
    ...
    print("\ncircHor: ", (sum(circHor_results) / float(len(circHor_results))) / 1024.0)
...
def main():
    #
    # (StrategyEnum.VerticalCoverageCircular,
